# Lucas Yang's VS Code Settings

* [`.vscode/settings.json`](.vscode/settings.json)
* [`.vscode/wsl-settings.json`](.vscode/wsl-settings.json) - `settings.json` for WSL
* [`.vscode/keybindings.json`](.vscode/keybindings.json)
* [`.vscode/frontend.code-snippets`](.vscode/frontend.code-snippets)
* [`.vscode/php.code-snippets`](.vscode/php.code-snippets)
* [`.vscode/extensions.json`](.vscode/extensions.json)

## User AI Prompts

* [`.github/prompts`](.github/prompts)

## Credit

* [Anthony's VS Code Settings](https://github.com/antfu/vscode-settings)
* [5 Annoying Things In VS Code You Can Fix Right Now](https://calebporzio.com/6-annoying-things-in-vs-code-you-can-fix-right-now)
